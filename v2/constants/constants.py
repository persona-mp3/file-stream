...
"""
Contains all information about status codes, request types and encoded formats
"""
...
#
# Operational response is used to represent exceptions that have occurred in the server itself, and bears
# a status code of '500'. This is solely to be used in where Exceptions might occur
#
# Packet-Status represents the state of each packet received by the server. It bears 200 and 400
# A 400 will also wait for the client to send that particular packet.
#
# Malformed-Packet bears a '401' code. Used when decoding a packet from the client.
#
# Corrupted-Response bears a '399' code. Used when decoding a packet from the client and the checksums don't match, 
# meaning data has been corrupted
#
# Disconnection-Request bears a '000' code. Used when a client wants to disconnect 
#
# Unsupported-Response bears a '444' code. Used when a client send an Unsupported version or request 
#
...
RETRY_RES = "Retry"
RETRY_CODE = "201"
ENC_RETRY_RES = RETRY_RES.encode(FORMAT)
ENC_RETRY_RES_LEN: bytes = struct.pack("B", len(ENC_RETRY_RES))
